...
static scm_t_bits evaluator_tag;	/* Unique identifier for Guile objects of evaluator type.  */

/*
 * Guile interface for libmatheval library.  Procedures below are simple wrappers for corresponding libmatheval procedures. 
 */
...
{
	/*
	 * Extend Guile with evaluator type and register procedure to free objects of this type. 
	 */
	...
	/*
	 * Interpret Guile code from file with name given through above argument. 
	 */
	scm_primitive_load(scm_makfrom0str(argv[1]));
}

/*
 * Program is demonstrating use of libmatheval library of procedures for evaluating mathematical functions.  Program expects single argument from command line and interpret Guile code (extended with procedures from libmatheval Guile interface) from this file. 
 */
int
main(int argc, char **argv)
{
	/*
	 * Initialize Guile library; inner_main() procedure gets called in turn. 
	 */
...
/*
 * Wrapper for evaluator_evaluate() procedure from libmatheval library.  Variable names and values are passed as lists from Guile, so copies of these argument have to be created in order to be able to call evaluator_evaluate() procedure. 
 */
static          SCM
...
/*
 * Wrapper for evaluator_get_variables() procedure from libmatheval library. 
 */
static          SCM
...
/*
 * Wrapper for evaluator_evaluate_x_y_z() procedure from libmatheval library. 
 */
static          SCM

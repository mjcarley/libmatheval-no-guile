...
typedef struct _Record {
	...
	char            type;	/* Symbol type ('v' for variable, 'f' for
				 * function).  */
	union {
		double          value;	/* Variable value.  */
		double          (*function) (double);	/* Pointer to function
							 * to calculate its
							 * value.  */
	}
	                data;
...
typedef struct {
	...
	int             reference_count;	/* Reference count for symbol
						 * table (evaluator for
						 * derivative uses same
						 * symbol table as evaluator
						 * for corresponding
						 * function).  */
}
                SymbolTable;
 ...
 */
Record         *
symbol_table_insert(SymbolTable * symbol_table, char *name,
		    char type,...);
